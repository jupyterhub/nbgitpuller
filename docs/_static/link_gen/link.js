...
function displayLink() {
  ...
  if (form.checkValidity()) {
    ...
    if (appName === "custom") {
      ...
      var repoName = new URL(repoUrl).pathname
        .split("/")
        .pop()
        .replace(/\.git$/, "");
      ...
      if (activeTab === "tab-auth-binder") {
        var contentRepoName = new URL(contentRepoUrl).pathname
          .split("/")
          .pop()
          .replace(/\.git$/, "");
        urlPath = apps[appName].generateUrlPath(
          contentRepoName + "/" + filePath
        );
      } else {
...
function toggleGitDiv() {
  document.getElementById("git-branch").style.display = "none";
}

function toggleGoogleDiv() {
  document.getElementById("git-branch").style.display = "";
}
 ...
 */
function render() {
  //Logic to show/hide branch text input box
  var fmsGoogle = document.getElementById("fms-google");
  fmsGoogle.addEventListener("change", toggleGitDiv, { passive: true });

  var fmsGit = document.getElementById("fms-github");
  fmsGit.addEventListener("change", toggleGoogleDiv, { passive: true });

  window.onload = toggleGitDiv;
  window.onload = toggleGoogleDiv;
...
function main() {
  ...
  document
    .querySelectorAll('#linkgenerator input[type="radio"]')
    .forEach(function (element) {
      element.addEventListener("change", render);
    });
  document
    .querySelectorAll(
      '#linkgenerator input[type="text"], #linkgenerator input[type="url"]'
    )
    .forEach(function (element) {
