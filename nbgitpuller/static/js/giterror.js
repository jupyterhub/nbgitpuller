export function GitError(gitsync, message) {
  ...
  if (s.includes("merge")) {
    ...
    return `<p class="lead">Content unavailable</p><p>The source content <a href=${repo}>${repo}</a> is unavailable.</p><p>This can be caused by:<ul><li>An invalid nbgitpuller link<ul><li>The source content could be private. You are not authorized to access the repository. This leads to a HTTP 403 "Forbidden" error. Check with the link author to grant public access to the source content.</li><li>The source content might not exist. This leads to a HTTP 404 "Not Found" error. Check with the link author that the source content exists, or if there is a typo in the source content repository link.</li></ul></li><li>Network issues<ul><li>Network issues can be caused by your internet service provider, your router device or your local machine's network settings. Check your local connection.</li><li>The service hosting the source content could be experiencing difficulties. This can lead to a HTTP 50x server error. Check the status page of the service, e.g. <a href="https://www.githubstatus.com/">GitHub status</a>, for the latest news on any incidents.</li></ul></li></ul></p><p>For general information, see the nbgitpuller<a href="https://nbgitpuller.readthedocs.io/en/latest/"> documentation.</a></p><hr><p><strong>Proceed without syncing</strong> to continue with the current state of your repository without any new updates.</p>`
  }
}
