...
class GitPuller(Configurable):
    ...
    def __init__(self, git_url, repo_dir, **kwargs):
        ...
        if self.branch_name is None:
            ...
            raise ValueError(
                f"Branch: {self.branch_name} -- not found in repo: {self.git_url}"
            )
    ...
    def reset_deleted_files(self):
        ...
        deleted_files = (
            subprocess.check_output(
                ["git", "ls-files", "--deleted", "-z"], cwd=self.repo_dir
            )
            .decode()
            .strip()
            .split("\0")
        ...
        for filename in deleted_files:
            ...
            if filename and filename not in upstream_deleted:
                yield from execute_cmd(
                    ["git", "checkout", f"origin/{self.branch_name}", "--", filename],
                    cwd=self.repo_dir,
                )
    ...
    def find_upstream_changed(self, kind):
        ...
        output = subprocess.check_output(
            ["git", "diff", f"..origin/{self.branch_name}", "--name-status"],
            cwd=self.repo_dir,
        ).decode()
        files = []
...
def main():
    ...
    parser.add_argument(
        "branch_name", default=None, help="Branch of repo to sync", nargs="?"
    )
    parser.add_argument(
